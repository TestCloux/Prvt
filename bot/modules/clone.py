...
def _clone(message, bot):
    ...
    if BOT_PM and message.chat.type != 'private':
        if EMOJI_THEME is True:
            ...
            pmwarn = f"<b>Files Sended To PM.</b>\n\n"
    ...
    if MIRROR_LOGS and message.chat.type != 'private':
        if EMOJI_THEME is True:
            ...
            logwarn = f"<b>Files Sended To <a href=\"{MIRROR_LOG_URL}\">Mirror Log channel</a> </b>\n\n"
    elif message.chat.type == 'private':
        logwarn = ''
    else:
        logwarn = ''
    ...
    if user_id != OWNER_ID and user_id not in SUDO_USERS:
            if TOTAL_TASKS_LIMIT == total_task:
                return sendMessage(f"<b>Dear {uname}\n\nThis Bot Has Reached Its Task Limit {TOTAL_TASKS_LIMIT}\n Your Tasks : {USER_TASKS_LIMIT} \n\n You Can Try your Tasks On Another Bots Too </b>", bot ,message)
    ...
    if is_gdrive_link(link):
        ...
        if EMOJI_THEME is True:
            ...
            cc = f'\n<b>• </b>Hey! {tag} My Job Completed ✨\n\n'
        if button in ["cancelled", ""]:
            sendMessage(f"{tag} {result}", bot, message)
        else:
            if PICS:
                msg = sendPhoto(result + cc + pmwarn + logwarn + warnmsg, bot, message, button)
            else:
